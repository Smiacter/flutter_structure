...
/// 包含：
///   普通转圈等待 - ViewStateLoadingWidget
///   无数据 - ViewStateEmptyWidget
///   错误页面 - ViewStateErrorWidget
///   未授权 - ViewStateUnAuthWidget
    ...
    switch (error.errorType) {
      case ViewStateErrorType.network:
        defaultImage = Transform.translate(
          offset: Offset(-50, 0),
          child: LoadAssetImage("common/network_error", width: 100, height: 100,),
        );
        defaultTitle = "网络连接异常,请检查网络或稍后重试";
        // errorMessage = ''; // 网络异常移除message提示
        break;
      case ViewStateErrorType.unknown:
        defaultImage = LoadAssetImage("common/page_error", width: 100, height: 100,);
        defaultTitle = "加载失败";
        break;
...
class ViewStateEmptyWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ViewStateWidget(
      onPressed: this.onPressed,
      image: image ?? LoadImage("assets/images/common/"),
      title: message ?? "空空如也",
      buttonText: buttonText,
      buttonTextData: "刷新一下",
    );
...
class ViewStateUnAuthImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Hero(
      tag: 'loginLogo',
      child: LoadAssetImage("common/default_avatar", width: 100, height: 100,),
    );
  }
}
...
class ViewStateWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        image ?? LoadAssetImage("common/page_error", width: 100, height: 100,),
        Padding(
